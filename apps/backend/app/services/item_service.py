...
class ItemService:
    ...
    async def create_item(
        ...
        full_item = await self.item_repository.get_with_relations(
    ...
    async def get_dashboard_stats(self, user_id: int) -> DashboardStats:
        today = date.today()
        total_items = await self.item_repository.count_total_items(user_id)
        active_warranties = await self.item_repository.count_active_warranties(
            user_id, today
        )
        expiring_soon = await self.item_repository.count_expiring_soon(
            user_id, today, settings.WARRANTY_EXPIRING_DAYS
        )
